  ...
  function insert_quick_jump_menu($currentforum = 0)
  {
    ...
    while ($arr = mysql_fetch_assoc($res))
    {
      if ($CURUSER["id_level"] >= $arr["minclassread"])
        print("<option value=forum.php?action=viewforum&forumid=" . $arr["id"] . ($currentforum == $arr["id"] ? " selected>" : ">") . htmlspecialchars(unesc($arr["name"])) . "</option>\n");
    }
...
if (!isset($forumid)) $forumid = 0;
  ...
  if ($action == "post")
  {
    ...
    if ($newtopic)
    {
      ...
      $subject = sqlesc(htmlspecialchars($subject));
  ...
  if ($action == "viewtopic")
  {
    ...
    $forum = htmlspecialchars(unesc($arr["name"]));
    ...
    while ($arr = mysql_fetch_assoc($res))
    {
      ...
      $res2 = mysql_query("SELECT username, level, avatar, uploaded, downloaded, name, flagpic FROM users INNER JOIN users_level ON users.id_level=users_level.id LEFT JOIN countries ON users.flag = countries.id WHERE users.id=$posterid") or sqlerr(__FILE__, __LINE__);
      ...
      if ($CURUSER["edit_forum"] == "yes")
      {
        ...
        while ($arr = mysql_fetch_assoc($res))
          if ($arr["id"] != $forumid && $CURUSER["id_level"] >= $arr["minclasswrite"])
            print("<option value=" . $arr["id"] . ">" . htmlspecialchars(unesc($arr["name"])) . "\n");
        ...
        else
        {
            ...
            for ($i = 1; $i <= $pages; ++$i)
                if ($page == $i)
                    ...
                    $pagemenu1 .= "<a href=\"forum.php?action=search&amp;keywords=" . htmlspecialchars($keywords) . "&amp;page=$i\"><b>$i</b></a>\n";
            if ($page == 1)
                $pagemenu2 = "<font class=gray><b>&lt;&lt; ".PREVIOUS."</b></font>\n";
            else
                $pagemenu2 = "<a href=\"forum.php?action=search&amp;keywords=" . htmlspecialchars($keywords) . "&amp;page=" . ($page - 1) . "\"><b>&lt;&lt; ".PREVIOUS."</b></a>\n";
            $pagemenu2 .= "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;\n";
            if ($page == $pages)
                $pagemenu2 .= "<font class=gray><b>".NEXT." &gt;&gt;</b></font>\n";
            else
                $pagemenu2 .= "<a href=\"forum.php?action=search&amp;keywords=" . htmlspecialchars($keywords) . "&amp;page=" . ($page + 1) . "\"><b>".NEXT." &gt;&gt;</b></a>\n";
            $offset = ($page * $perpage) - $perpage;
