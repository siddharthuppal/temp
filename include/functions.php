...
function print_version()
{
  ...
  print("BtiTracker ($tracker_version) by <a href=\"http://www.btiteam.org\">Btiteam</a></p>");
...
function userlogin() {
  ...
  //  $res = mysql_query("SELECT * FROM bannedip WHERE $nip >='first' AND $nip <='last' ") or sqlerr(__FILE__, __LINE__);
   /* if (mysql_num_rows($res) > 0)
    ...
    $res = mysql_query("SELECT users.topicsperpage, users.postsperpage,users.torrentsperpage, users.flag, users.avatar, UNIX_TIMESTAMP(users.lastconnect) AS lastconnect, UNIX_TIMESTAMP(users.joined) AS joined, users.id as uid, users.username, users.password, users.random, users.email, users.language,users.style, users.time_offset, users_level.* FROM users INNER JOIN users_level ON users.id_level=users_level.id WHERE users.id = $id") or die(mysql_error());
...
function print_news($limit=0)
     ...
     $output="";
         ...
         while ($rows=mysql_fetch_array($res))
               ...
               $output = eregi_replace("\{news_date}", date("d/m/Y H:i",$rows["news_date"]-$offset), $output);
...
function print_users()
     ...
     if (!isset($_GET["level"])) $_GET["level"] = "";
         ...
         if ($search!="")
            {
            $where=" AND users.username LIKE '%".htmlspecialchars(mysql_escape_string($_GET["searchtext"]))."%'";
