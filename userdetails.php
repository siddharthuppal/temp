...
if ($id>1) {
   $res=mysql_query("SELECT users.avatar,users.email,users.cip,users.username,users.downloaded,users.uploaded,UNIX_TIMESTAMP(users.joined) as joined,UNIX_TIMESTAMP(users.lastconnect) as lastconnect,users_level.level, users.flag, countries.name, countries.flagpic, users.pid, users.time_offset FROM users INNER JOIN users_level ON users_level.id=users.id_level LEFT JOIN countries ON users.flag=countries.id WHERE users.id=$id");
...
if ($row["avatar"] && $row["avatar"]!="")
   print("<td class=lista align=center valign=middle rowspan=4><img border=0 src=".htmlspecialchars($row["avatar"])." /></td>");
print("</tr>");
...
if ($resuploaded && mysql_num_rows($resuploaded)>0)
   ...
   while ($rest=mysql_fetch_array($resuploaded))
         ...
         if ($GLOBALS["usepopup"])
         {
            print("\n<tr>\n<td class=\"lista\"><a href=\"javascript:popdetails('details.php?id=".$rest{"info_hash"}."')\">".unesc($rest["filename"])."</td>");
            include("include/offset.php");
            print("\n<td class=\"lista\" align=\"center\">".date("d/m/Y",$rest["added"]-$offset)."</td>");
            print("\n<td class=\"lista\" align=\"center\">".makesize($rest["size"])."</td>");
            print("\n<td align=\"center\" class=\"".linkcolor($rest["seeds"])."\"><a href=\"javascript:poppeer('peers.php?id=".$rest{"info_hash"}."')\">$rest[seeds]</td>");
            print("\n<td align=\"center\" class=\"".linkcolor($rest["leechers"])."\"><a href=\"javascript:poppeer('peers.php?id=".$rest{"info_hash"}."')\">$rest[leechers]</td>");
            if ($rest["finished"]>0)
               print("\n<td align=\"center\" class=\"lista\"><a href=\"javascript:poppeer('torrent_history.php?id=".$rest["info_hash"]."')\">" . $rest["finished"] . "</a></td>");
            else
         ...
         if ($torlist->ip !="")
           {
             if ($GLOBALS["usepopup"])
             {
                 print("\n<tr>\n<td class=\"lista\"><a href=\"javascript:popdetails('details.php?id=".$torlist->infohash."')\">".unesc($torlist->filename)."</td>");
                 ...
                 print("\n<td align=\"center\" class=\"".linkcolor($torlist->seeds)."\"><a href=\"javascript:poppeer('peers.php?id=".$torlist->infohash."')\">$torlist->seeds</td>");
                 print("\n<td align=\"center\" class=\"".linkcolor($torlist->leechers)."\"><a href=\"javascript:poppeer('peers.php?id=".$torlist->infohash."')\">$torlist->leechers</td>");
                 print("\n<td align=\"center\" class=\"lista\"><a href=\"javascript:poppeer('torrent_history.php?id=".$torlist->infohash."')\">".$torlist->finished."</td>\n</tr>");
             }
             else
             {
    ...
    while ($torlist = mysql_fetch_object($anq))
        {
            if ($GLOBALS["usepopup"])
            {
                print("\n<tr>\n<td class=\"lista\"><a href=\"javascript:popdetails('details.php?id=".$torlist->info_hash."')\">".unesc($torlist->filename)."</td>");
                print("\n<td class=\"lista\" align=\"center\">".makesize($torlist->size)."</td>");
                ...
                if ($torlist->downloaded>0)
                     ...
                     $peerratio="oo";
                print("\n<td align=\"center\" class=\"lista\">".unesc($peerratio)."</td>");
                print("\n<td align=\"center\" class=\"".linkcolor($torlist->seeds)."\"><a href=\"javascript:poppeer('peers.php?id=".$torlist->info_hash."')\">$torlist->seeds</td>");
                print("\n<td align=\"center\" class=\"".linkcolor($torlist->leechers)."\"><a href=\"javascript:poppeer('peers.php?id=".$torlist->info_hash."')\">$torlist->leechers</td>");
                print("\n<td align=\"center\" class=\"lista\"><a href=\"javascript:poppeer('torrent_history.php?id=".$torlist->info_hash."')\">".$torlist->finished."</td>\n</tr>");
            }
            else
            {
